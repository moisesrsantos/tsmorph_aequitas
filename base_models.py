...
class Base_Models:
    ...
    def lstm(self):
        ...
        model = LSTM(h=self.h, max_steps=500, enable_checkpointing=False, logger=False)
    ...
    def nhits(self):
        ...
        Y_df = Y_df.rename(columns={'index': 'ds', Y_df.iloc[:,1].name: 'y'})
        Y_df = Y_df[['unique_id', 'ds', 'y']]
        model = NHITS(h=self.h, max_steps=500, input_size=2 * self.h, enable_checkpointing=False, logger=False)
        nf = NeuralForecast(models=[model], freq='D')
        nf.fit(df=Y_df)
